(function () {
  ...
  function roleCompatible(person, service) {
    return person.role === "vet" ? service.vetShare > 0 : service.supportShare > 0;
  }

  function personQualified(person, service) {
    const required = person.role === "vet" ? service.vetSkills : service.supportSkills;
    return required.every((skill) => person.skills.includes(skill));
  }
  ...
  function normalizeAllocations(person, allowPartial = false) {
    const source = Array.isArray(person.allocations) && person.allocations.length ? person.allocations : legacyAllocations(person);
    const merged = new Map();
    source.slice(0, 3).forEach((row) => {
      const service = SERVICE_BY_ID[row?.serviceId];
      if (!service || !roleCompatible(person, service)) return;
      const share = clamp(Number(row.share || 0), 0, 1);
      merged.set(service.id, (merged.get(service.id) || 0) + share);
    });
    let allocations = [...merged.entries()].map(([serviceId, share]) => ({ serviceId, share }));
    if (!allocations.length) allocations = legacyAllocations(person);
    const total = allocations.reduce((sum, row) => sum + row.share, 0);
    if (!allowPartial && total > 0) allocations = allocations.map((row) => ({ ...row, share: row.share / total }));
    return allocations.slice(0, 3);
  }

  function validAllocations(person, allocations) {
    if (!Array.isArray(allocations) || !allocations.length || allocations.length > 3) return false;
    const ids = new Set();
    let total = 0;
    for (const row of allocations) {
      const service = SERVICE_BY_ID[row?.serviceId];
      const share = Number(row?.share);
      if (!service || !roleCompatible(person, service) || ids.has(service.id) || share <= 0 || share > 1) return false;
  ...
  function emptyEffects() {
    return { oneTimeCosts: 0, cashAdjustment: 0, vetTrainingHours: 0, supportTrainingHours: 0, supportReserved: 0, facilityHoursLost: 0, relocationLoss: 0, advancedDemand: 0, routineDemand: 0, recruitment: [] };
  }
  ...
  function applyAction(target, action, chargeCosts = false) {
    ...
    if (p.kind === "training") {
      ...
      if (training && !target.trainings.includes(p.targetId)) {
  ...
  function missingRequirements(service, clinic) {
    ...
    service.supportSkills.forEach((id) => { if (!hasSkill(clinic, "support", id)) missing.push({ type: "missingSupportSkill", id }); });
  ...
  function initialResources(clinic, effects) {
    ...
    const preliminary = clinic.staff.map((person) => {
      ...
      const expectedAbsenceHours = contractedHours * hr.absenteeism;
      const gross = contractedHours - expectedAbsenceHours;
      const training = person.role === "vet" ? effects.vetTrainingHours : effects.supportTrainingHours;
      const sameRoleCount = Math.max(1, clinic.staff.filter((item) => item.role === person.role).length);
      return { person, contractedHours, expectedAbsenceHours, grossHours: gross, trainingHours: training / sameRoleCount, beforeDuties: Math.max(0, gross - training / sameRoleCount) };
    });
    ...
    const staffRows = preliminary.map((row) => {
      ...
      const allocations = normalizeAllocations(person, true);
  ...
  function vetDuration(service, clinic) {
    let value = service.duration * service.vetShare;
  ...
  function supportDuration(service, clinic) {
    let value = service.duration * service.supportShare;
  ...
  function projectedDemand(service, clinic, effects = emptyEffects()) {
    ...
    return Math.max(0, Math.round(
      service.demand * (clinic.clients / 1200) * segment.serviceMix[service.id] * priceEffect *
  ...
  function lowestCap(caps) {
    ...
    if (key.startsWith("room:")) return { type: "room", id: key.split(":")[1] };
    if (key.startsWith("equipment:")) return { type: "equipment", id: key.split(":")[1] };
    return { type: "demand" };
  }
  ...
  function simulateYear(clinic, before, effects, actions) {
    ...
    D.services.slice().sort((a, b) => a.priority - b.priority).forEach((service) => {
      const serviceState = clinic.services[service.id];
      ...
      if (serviceState.active) {
        ...
        if (!missing.length) {
          const caps = {
            demand,
            vet: vd > 0 ? Math.floor((resources.vetByService[service.id] || 0) / vd) : 999999,
            support: sd > 0 ? Math.floor((resources.supportByService[service.id] || 0) / sd) : 999999
          };
          service.roomIds.forEach((id) => { caps[`room:${id}`] = Math.floor((resources.rooms[id] || 0) / service.duration); });
          service.equipmentIds.forEach((id) => { caps[`equipment:${id}`] = Math.floor((resources.equipment[id] || 0) / service.duration); });
          honored = Math.max(0, Math.min(...Object.values(caps)));
          bottleneck = honored >= demand ? { type: "demandMet" } : lowestCap(caps);
          const vetUsed = honored * vd;
          const supportUsed = honored * sd;
          ...
          service.roomIds.forEach((id) => { resources.rooms[id] = Math.max(0, resources.rooms[id] - honored * service.duration); });
          service.equipmentIds.forEach((id) => { resources.equipment[id] = Math.max(0, resources.equipment[id] - honored * service.duration); });
        } else bottleneck = missing[0];
      ...
      results.push({ id: service.id, active: serviceState.active, demand, honored, price: serviceState.price, revenue: serviceRevenue, variableCosts: variable, contributionPerCase: serviceState.price * (1 - service.variableCost), missing, bottleneck, vetDuration: vd, supportDuration: sd });
    });
    const serviceHourRows = [];
    results.forEach((result) => {
      const service = SERVICE_BY_ID[result.id];
      [["vet", result.vetDuration, resources.startVetByService], ["support", result.supportDuration, resources.startSupportByService]].forEach(([role, duration, starts]) => {
        if (!(role === "vet" ? service.vetShare : service.supportShare)) return;
        ...
        const effectiveHours = starts[result.id] || 0;
        const neededHours = result.active ? result.demand * duration : 0;
        const usedHours = result.honored * duration;
        serviceHourRows.push({ serviceId: result.id, role, assignedHours, effectiveHours, blockedHours: Math.max(0, assignedHours - effectiveHours), neededHours, usedHours, shortageHours: Math.max(0, neededHours - effectiveHours), surplusHours: Math.max(0, effectiveHours - neededHours) });
      });
    });
    resources.staffRows.forEach((staffRow) => {
      staffRow.assignments.forEach((assignment) => {
        ...
        const duration = staffRow.role === "vet" ? result?.vetDuration : result?.supportDuration;
        ...
        const serviceUsed = (result?.honored || 0) * (duration || 0);
      ...
      staffRow.unusedHours = Math.max(0, staffRow.availableHours - staffRow.usedHours - staffRow.blockedHours);
    ...
    const socialCharges = payroll * .22;
    ...
    social.clientTrust = clamp(social.clientTrust + honoredRate * 3.2 - (1 - honoredRate) * 5.8 + communication.trust + accessTrust, 0, 100);
    social.staffClimate = clamp(social.staffClimate + hr.climate + periodClimate + payClimate + (staffUse > .94 ? -3 : staffUse >= .45 ? 1 : -1), 0, 100);
    ...
    const segment = D.segments[clinic.marketFocus];
    const reputation = clamp(clinic.reputation + honoredRate * 4 - (1 - honoredRate) * 5 + (netResult >= 0 ? .8 : -.7) + (social.clientTrust - 50) / 75, 20, 95);
    ...
    return {
      ...
      financial: { revenue, baseVariableCosts, variableCosts, payroll, socialCharges, ownedMaintenance, leaseCosts, roomRent, locationRent: location.rent, facilityCosts, openingCosts, dropoffCost, stockCost: stock.cost, hrCost, marketingCost, sustainabilityCost, admin, loanPayment, loanPrincipalPayment, loanInterest, oneTimeCosts: effects.oneTimeCosts, cashAdjustment: financingCashAdjustment, fixedCosts, totalCosts, operatingResult, tax, netResult, treasury, margin: revenue ? netResult / revenue : 0, breakEvenCases: avgContribution ? Math.ceil(fixedCosts / avgContribution) : 0 },
      operational: { totalDemand, totalOpportunity, totalHonored, honoredRate, mainConstraint, staffUse, startVetHours: resources.totalVet, remainingVetHours: resources.remainingVet, startSupportHours: resources.totalSupport, remainingSupportHours: resources.remainingSupport, reservedSupportHours: resources.reservedSupport, roomUse: resourceUse(resources.startRooms, resources.rooms), equipmentUse: resourceUse(resources.startEquipment, resources.equipment), advancedServed, readyServices: D.services.filter((service) => clinic.services[service.id].active && !missingRequirements(service, clinic).length).length, staffRows: resources.staffRows, serviceHourRows },
      social: { before: clone(before.social || DEFAULT_SOCIAL), after: social },
      carbon,
  ...
  function blockerText(reason) {
    ...
    if (type === "room" || type === "missingRoom") return t(`blockers.${type}`, { name: roomName(reason.id) });
    if (type === "equipment" || type === "missingEquipment") return t(`blockers.${type}`, { name: equipmentName(reason.id) });
  ...
  function getBeginnerSignals(forecast, clinic) {
    ...
    if (forecast.operational.staffUse < .65) {
      signals.push({
        key: "teamUnderused", status: "warn", destination: { drawer: "services" },
        title: L("Much of the team’s paid time is unused", "Une grande partie du temps payé de l’équipe est inutilisée"),
        text: L(
          `${pct(1 - forecast.operational.staffUse)} of available work hours are unused. Compatible services could use some of this time.`,
          `${pct(1 - forecast.operational.staffUse)} des heures de travail disponibles sont inutilisées. Des services compatibles pourraient employer une partie de ce temps.`
        ),
        action: L("Explore compatible services", "Explorer les services compatibles")
      });
    }
    const constraint = forecast.operational.mainConstraint;
    if (!["demandMet", "notOffered", "none"].includes(constraint.type)) {
      signals.push({
        key: "capacityBlocked", status: "warn", destination: { drawer: "services" },
        title: L("Some requested care cannot be delivered", "Certains soins demandés ne peuvent pas être fournis"),
        text: blockerText(constraint), action: L("Address the blocker", "Traiter le blocage")
      });
    }
    if (signals.length < 3) {
  ...
  function consequencePreview(key, payload) {
    ...
    return `<div class="consequence-grid" aria-label="${escapeHtml(L("Expected consequences", "Conséquences attendues"))}">${cells.map(([label, value, word]) => `<div><span>${escapeHtml(label)}</span><strong>${escapeHtml(value)}</strong><em>${escapeHtml(word)}</em></div>`).join("")}</div>`;
  }
  ...
  function renderOverview(planned, forecast) {
    ...
    return `<section class="page"><div class="page-heading"><div><h1>${escapeHtml(L("Clinic overview", "Vue d’ensemble de la clinique"))}</h1><p>${escapeHtml(L("See the situation, choose one area, and check the consequences before acting.", "Observez la situation, choisissez un domaine et vérifiez les conséquences avant d’agir."))}</p></div></div>
      ...
      <section class="card-section"><div class="panel-heading"><div><h2>${escapeHtml(L("What needs attention", "Points d’attention"))}</h2><p>${escapeHtml(L("Each signal explains what is happening and where you can investigate it.", "Chaque signal explique ce qui se passe et où l’examiner."))}</p></div></div><div class="priority-list">${signals.map((item) => `<article class="signal-${item.status}" data-signal-key="${item.key}"><div><span>${escapeHtml(item.title)}</span><strong>${escapeHtml(item.text)}</strong></div><button class="button secondary" ${item.destination.drawer ? `data-open-drawer="${item.destination.drawer}"` : `data-domain="${item.destination.domain}"`}>${escapeHtml(item.action)}</button></article>`).join("")}</div></section>
  ...
  function renderCare(planned, forecast) {
    ...
    const ready = active.filter((service) => !missingRequirements(service, planned).length).length;
    ...
    return `<section class="page"><div class="page-heading"><div><h1>${escapeHtml(L("Care & facilities", "Soins et installations"))}</h1><p>${escapeHtml(L("Start from what is active; open the catalog only when you want to change something.", "Partez de l’existant ; ouvrez le catalogue uniquement pour effectuer un changement."))}</p></div></div>
      <div class="dashboard-grid compact-four">${metricCard(L("Active services", "Services actifs"), number(active.length), `${ready} ${L("ready", "prêts")}`)}${metricCard(L("Cases served", "Cas traités"), number(forecast.operational.totalHonored), pct(forecast.operational.honoredRate))}${metricCard(L("Rooms", "Salles"), number(roomCount))}${metricCard(L("Clinical equipment", "Équipement clinique"), number(equipmentCount))}</div>
      <section class="card-section"><div class="panel-heading"><div><h2>${escapeHtml(L("Current services", "Services actuels"))}</h2><p>${escapeHtml(L("Readiness and the current bottleneck are shown without exposing the whole catalog.", "La préparation et le blocage actuel sont visibles sans afficher tout le catalogue."))}</p></div><button class="button primary" data-open-drawer="services">${escapeHtml(L("Explore services", "Explorer les services"))}</button></div><div class="summary-list">${active.map((service) => { const row = forecast.serviceResults.find((item) => item.id === service.id); return `<article><div><strong>${escapeHtml(serviceName(service.id))}</strong><span>${escapeHtml(missingRequirements(service, planned).length ? L("Blocked", "Bloqué") : L("Ready", "Prêt"))}</span></div><span>${number(row.honored)} / ${number(row.demand)} ${escapeHtml(L("served", "traités"))}</span><em>${escapeHtml(blockerText(row.bottleneck))}</em></article>`; }).join("")}</div></section>
  ...
  function renderTeam(planned, forecast) {
    ...
    return `<section class="page"><div class="page-heading"><div><h1>${escapeHtml(L("Team & operations", "Équipe et opérations"))}</h1><p>${escapeHtml(L("See the team first; reveal applicants and operational alternatives only when needed.", "Voyez d’abord l’équipe ; affichez les candidats et les options uniquement si nécessaire."))}</p></div><button class="button primary" data-open-drawer="recruitment">${escapeHtml(L("Post a vacancy", "Publier une offre"))}</button></div>
      ...
      <section class="card-section"><div class="panel-heading"><div><h2>${escapeHtml(L("Current team", "Équipe actuelle"))}</h2><button class="text-button" data-open-drawer="hoursByService">${escapeHtml(L("See hours by service", "Voir les heures par service"))} ›</button></div><button class="button secondary" data-open-drawer="training">${escapeHtml(L("Plan training", "Planifier une formation"))}</button></div><div class="staff-grid">${planned.staff.map((person) => { const row = rows[person.id]; const summary = normalizeAllocations(person).map((allocation) => `${serviceName(allocation.serviceId)} ${Math.round(allocation.share * 100)}%`).join(" · "); return `<article class="staff-card"><div class="staff-head"><div><h3>${escapeHtml(person.name)}</h3><span>${escapeHtml(person.role === "vet" ? L("Veterinarian", "Vétérinaire") : L("Support", "Soutien"))}</span></div><strong>${pct(row?.workload || 0)}</strong></div><p class="allocation-summary">${escapeHtml(summary)}</p><div class="staff-hours-line"><span>${escapeHtml(L("Hours used", "Heures utilisées"))}</span><strong>${number(row?.usedHours || 0)} / ${number(row?.availableHours || 0)}</strong></div>${meter(row?.workload || 0, row?.workload > .94 ? "warn" : "good")}<div class="button-row"><button class="button primary" data-open-drawer="staffAllocation" data-context="${person.id}">${escapeHtml(L("Change time allocation", "Modifier l’affectation du temps"))}</button><button class="button secondary" data-open-drawer="staffPerson" data-context="${person.id}">${escapeHtml(L("Pay", "Salaire"))}</button></div></article>`; }).join("")}</div></section>
  ...
  function renderDrawerBody(planned, forecast) {
    ...
    if (ui.drawer === "training") return `<div class="drawer-cards">${Object.entries(D.trainings).map(([id, training]) => { const trained = planned.trainings.includes(id); const payload = { kind: "training", targetId: id }; return `<article class="choice-card"><h3>${escapeHtml(itemLabel(training.name))}</h3><p>${money(training.cost)} ${escapeHtml(L("once", "une fois"))} · ${number(training.hours)} ${escapeHtml(L("training hours", "heures de formation"))}</p>${trained ? `<strong>✓ ${escapeHtml(L("Already trained", "Formation déjà acquise"))}</strong>` : `${consequencePreview(`training:${id}`, payload)}${reviewButton(`training:${id}`, payload)}`}</article>`; }).join("")}</div>`;
    ...
    if (ui.drawer === "pricing") return `<p>${escapeHtml(L("Price sensitivity shows how strongly requests may change when the price changes.", "La sensibilité au prix indique dans quelle mesure les demandes peuvent changer lorsque le prix évolue."))}</p><div class="price-editor">${D.services.filter((service) => planned.services[service.id].active).map((service) => { const current = ui.decisionDrafts[`price:${service.id}`] ?? planned.services[service.id].price; return `<article><div><strong>${escapeHtml(serviceName(service.id))}</strong><span>${escapeHtml(L("Price sensitivity", "Sensibilité au prix"))}: ${escapeHtml(priceSensitivity(service.elasticity))}</span><span>${escapeHtml(L("Money left after direct supplies", "Argent restant après les fournitures directes"))}: ${money(current * (1 - service.variableCost))}</span></div><label><span>${escapeHtml(L("Price", "Prix"))}</span><input type="number" min="1" max="5000" value="${current}" data-draft-price="${service.id}" data-draft-key="price:${service.id}"></label><button class="button primary" data-review-price="${service.id}">${escapeHtml(L("Review price", "Examiner le prix"))}</button></article>`; }).join("")}</div>`;
  ...
  function buildPrintableReportHtml() {
    ...
    const yearSections = state.history.map((report, reportIndex) => {
      ...
      const staffTable = staffRows.length ? reportTable([L("Person", "Personne"), L("Available", "Disponibles"), L("Used", "Utilisées"), L("Unused", "Inutilisées"), L("Blocked", "Bloquées"), L("Workload", "Charge"), L("Allocations", "Affectations")], staffRows.map((row) => { const person = snapshotStaff.find((item) => item.id === row.id); return [escapeHtml(person?.name || row.id), number(row.availableHours), number(row.usedHours), number(row.unusedHours), number(row.blockedHours), pct(row.workload), escapeHtml((row.assignments || []).map((assignment) => `${serviceName(assignment.serviceId)} ${Math.round(assignment.share * 100)}%`).join(" · "))]; })) : `<p class="notice">${escapeHtml(L("Detailed staff allocation was not recorded for this migrated year.", "L’affectation détaillée du personnel n’a pas été enregistrée pour cette année migrée."))}</p>`;
  ...
  document.addEventListener("click", (event) => {
    ...
    if (button.dataset.allocationAdjust) {
      ...
      draft[index].share = clamp(Math.round((draft[index].share * 100 + Number(button.dataset.allocationAdjust)) / 5) * 5 / 100, 0, 1);
    ...
    if (button.dataset.addAllocation !== undefined) {
      ...
      const draft = allocationDraftFor(person);
      if (draft.length < 3 && !draft.some((row) => row.serviceId === serviceId)) draft.push({ serviceId, share: 0 });
