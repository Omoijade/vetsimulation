(function () {
  ...
  function projectedDemand(service, clinic, effects = emptyEffects()) {
    ...
    const priceEffect = priceRatio <= 1
      ...
      // of its market. Exponential decay keeps the curve strictly decreasing at every price while
      // matching the old linear slope near the willingness point, where all calibration sits.
      : Math.exp(-(priceRatio - 1) * service.elasticity * 2.2 * segment.priceSensitivity);
    const monitoring = D.marketingStrategies.monitoring[clinic.marketing.monitoring];
